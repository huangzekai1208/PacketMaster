...
- chat 诊断完成后必须将完整 `report.json` 确定性整理为自然语言，不为报告转述额外调用模型。
...
- [ ] 实现结构化报告到中文自然语言的确定性叙述器，覆盖分析概览、主因、全部候选原因、关键证据、限制、排查步骤和优化建议。
- [ ] 每条候选原因展示百分数置信度、支持证据、反向证据、缺失证据和建议；空字段自然省略。
- [ ] 报告叙述不得调用模型、补充报告外事实或泄露内部元数据和敏感字段；仅允许显示用户需要的 JSON 报告保存路径。
- [ ] 诊断完成后自动在聊天框输出完整自然语言报告，并在结尾显示 JSON 报告路径。
- [ ] 实现 `/report` 重复渲染相同自然语言报告，`/evidence` 输出有界证据引用。
- [ ] 实现 `/save`、`/help`、`/new` 和 `/quit` 的中文输出。
- [ ] 问答失败后保持提示符可用，允许用户继续查看报告或开始新任务。
- [ ] 覆盖空格路径、中文路径、相对路径和 Windows 盘符路径。
- [ ] 覆盖 EOF、Ctrl+C、空问题和未知命令。
- [ ] 测试完整报告、空可选字段、unresolved、降级报告和多个候选原因的自然语言渲染。
...
- [ ] 检查诊断完成后自动输出完整自然语言报告，且内容与落盘 `report.json` 一致。
- [ ] 检查 `/report` 可重复输出相同诊断事实，不触发新的模型调用。
...
- 完整 `report.json` 会在聊天框中被确定性整理为中文自然语言，`/report` 可重复查看。
