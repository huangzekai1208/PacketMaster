...
目标：在保留现有 `packetmaster diagnose` 行为的基础上，新增 `packetmaster chat`。用户通过引导式输入完成一次 TCP 测速诊断，并围绕当前报告和本地证据持续问答。

依据：`docs/superpowers/specs/2026-07-24-packetmaster-cli-chat-design.md`

技术路线：复用现有诊断 LangGraph、FastMCP 工具、Real Adapter、DiagnosisContext 和 DeepSeek 结构化输出兼容层；新增独立的会话管理与有界问答 LangGraph。首次诊断执行全量流式分析，后续问答只复用分析产物和分页证据，不重新扫描完整报文。

## 全局约束

- `packetmaster diagnose` 的参数、输出和报告契约必须保持兼容。
- `packetmaster chat` 默认分析 download，只有用户明确选择时使用 upload 或 both。
- 参数收集采用确定性 CLI 校验，不使用模型解析路径和带宽。
